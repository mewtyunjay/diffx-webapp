...
export async function buildCodeReviewPromptContext(): Promise<CodeReviewPromptContext> {
  ...
  if (files.length === 0) {
    return {
      sourceFingerprint,
      focusFiles: [],
      promptContext: "No changed files were available for code review.",
    };
  }
  ...
  return {
    sourceFingerprint,
    focusFiles: files.map((file) => file.path),
