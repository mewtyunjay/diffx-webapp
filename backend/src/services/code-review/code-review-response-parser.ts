...
export function parseCodeReviewPayloadFromResponse(raw: unknown, providerName: string): unknown {
  ...
  if (!responseText) {
    throw new ApiRouteError(
      ...
      `${providerName} returned an empty code review response.`,
    );
  }
  ...
  if (!jsonObject) {
    throw new ApiRouteError(
      502,
      "REVIEW_GENERATION_FAILED",
      `${providerName} response did not include a JSON findings payload.`,
    );
  }
  ...
  try {
    ...
    throw new ApiRouteError(
      502,
      "REVIEW_GENERATION_FAILED",
      `${providerName} JSON findings payload could not be parsed.`,
    );
  }
}
